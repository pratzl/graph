...
TEST_CASE("daa vertex functions", "[daa][vertex][functions]") {
  ...
  vertex_iterator_t<Graph> f3 = ::ranges::find_if(g, [](vertex_t<Graph>& u2) { return u2.name == "Frankfurt"; });
...
TEST_CASE("daa edge functions", "[daa][edge][functions]") {
  ...
  vertex_iterator_t<Graph> u = ::ranges::find_if(g, [](vertex_t<Graph>& u2) { return u2.name == "Frankfurt"; });
...
TEST_CASE("daa dfs transitive closure", "[daa][dfs][transitive closure]") {
  ...
  */
...
#elif TEST_OPTION == TEST_OPTION_TEST
  {
    ...
    EXPECT_EQ("Munchen", reaches_vec[0].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[1].from->name);
    EXPECT_EQ("Mannheim", reaches_vec[1].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[2].from->name);
    EXPECT_EQ("Karlsruhe", reaches_vec[2].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[3].from->name);
    EXPECT_EQ("Augsburg", reaches_vec[3].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[4].from->name);
    EXPECT_EQ("Munchen", reaches_vec[4].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[5].from->name);
    EXPECT_EQ("Wurzburg", reaches_vec[5].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[6].from->name);
    EXPECT_EQ("Erfurt", reaches_vec[6].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[7].from->name);
    EXPECT_EQ("Nurnberg", reaches_vec[7].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[8].from->name);
    EXPECT_EQ("Stuttgart", reaches_vec[8].to->name);

    EXPECT_EQ("Frankfurt", reaches_vec[9].from->name);
    ...
    EXPECT_EQ("Munchen", reaches_vec[11].to->name);

    EXPECT_EQ("Kassel", reaches_vec[12].from->name);
    EXPECT_EQ("Munchen", reaches_vec[12].to->name);
    ...
    EXPECT_EQ("Munchen", reaches_vec[15].to->name);

    EXPECT_EQ("Nurnberg", reaches_vec[16].from->name);
    EXPECT_EQ("Stuttgart", reaches_vec[16].to->name);

    EXPECT_EQ("Nurnberg", reaches_vec[17].from->name);
    EXPECT_EQ("Munchen", reaches_vec[17].to->name);

    EXPECT_EQ("Wurzburg", reaches_vec[18].from->name);
    EXPECT_EQ("Erfurt", reaches_vec[18].to->name);

    EXPECT_EQ("Wurzburg", reaches_vec[19].from->name);
    EXPECT_EQ("Nurnberg", reaches_vec[19].to->name);

    EXPECT_EQ("Wurzburg", reaches_vec[20].from->name);
    EXPECT_EQ("Stuttgart", reaches_vec[20].to->name);

    EXPECT_EQ("Wurzburg", reaches_vec[21].from->name);
    EXPECT_EQ("Munchen", reaches_vec[21].to->name);
  }
#endif
}
...
TEST_CASE("daa warshall transitive closure", "[daa][warshall][transitive closure]") {
  ...
  */
...
#elif TEST_OPTION == TEST_OPTION_TEST
  {
    ...
    EXPECT_EQ("Frankfurt", reaches_vec[6].from->name);
    EXPECT_EQ("Munchen", reaches_vec[6].to->name);
    ...
    EXPECT_EQ("Frankfurt", reaches_vec[9].from->name);
    EXPECT_EQ("Wurzburg", reaches_vec[9].to->name);
    ...
    EXPECT_EQ("Nurnberg", reaches_vec[16].from->name);
    EXPECT_EQ("Munchen", reaches_vec[16].to->name);
    ...
    EXPECT_EQ("Wurzburg", reaches_vec[19].from->name);
    EXPECT_EQ("Munchen", reaches_vec[19].to->name);

    EXPECT_EQ("Wurzburg", reaches_vec[20].from->name);
    EXPECT_EQ("Nurnberg", reaches_vec[20].to->name);
