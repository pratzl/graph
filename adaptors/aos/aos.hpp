...
public:
  ...
  public:
    ...
    using difference_type =
          typename std::iterator_traits<std::tuple_element_t<0, std::tuple<RandomAccessIterators...>>>::difference_type;
    using pointer           = value_type*;
    using iterator_category = std::random_access_iterator_tag;

    auto operator*() { return star_helper(std::make_integer_sequence<size_t, sizeof...(RandomAccessIterators)>()); }
    auto operator[](size_t i) {
      return bracket_helper(i, std::make_integer_sequence<size_t, sizeof...(RandomAccessIterators)>());
    }
