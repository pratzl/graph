...
std::tuple<double, std::vector<tree_mem>> bk_maxflow(Graph& A, std::vector<double>& cap) {
  ...
  vertex_index_t terminal = source + 1;
  double         max_flow = 0.0;
  double         tol      = 1e-12;
  size_t         gtime    = 0;
  ...
  for (auto p = 0; p < n_vtx; ++p) {
    for (auto it = G[p].begin(); it != G[p].end(); ++it) {
      ...
      if (std::get<2>(*it) == NULL) {
        ...
        // find p in q's adjacencies
        for (; std::get<0>(*backp) != p; ++backp)
          ;
        std::get<2>(*it)    = &(std::get<1>(*backp));
        std::get<2>(*backp) = &(std::get<1>(*it));
      }
    }
  }
  ...
  while (true) {
    ...
    *back_residual_cap += min_cap;
    ...
    while (!orphans.empty()) {
      ...
      tree_mem                     t = tree_id[p];
      vertex_index_t               min_pred{null_vertex};
      ...
      if (min_pred == null_vertex) {
        ...
        for (auto it = G[p].begin(); it != G[p].end(); ++it) {
          ...
          if (tree_id[q] == tree_mem::source && std::get<1>(*it) > tol) {
            active.push(q);
          } else if (tree_id[q] == tree_mem::term && *std::get<2>(*it) > tol) {
            active.push(q);
          }
